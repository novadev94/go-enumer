...
func TestE2E_CLI(t *testing.T) {
	...
	testcases := []struct {
		...
		{"standard serializers",
			"greeting", []string{"-serializers=binary-str,graphql,json,sql,text,yaml"}, "gen.serializers.golden"},
		{"standard serializers - different order, same result",
			"greeting", []string{"-serializers=sql,graphql,json,yaml,binary-str,text"}, "gen.serializers.golden"},
		{"serializers and yaml.v3",
			"greeting", []string{"-serializers=binary-str,graphql,json,sql,text,yaml.v3"}, "gen.serializers.yaml_v3.golden"},
		{"standard serializers - deserialize with ignore case",
			"greeting", []string{"-serializers=sql,graphql,json,yaml,binary-str,text", "-support=ignore-case"}, "gen.serializers.ignore-case.golden"},
